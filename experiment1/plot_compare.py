...
- experiment1/bench/qiea_results.csv

and writes PNG plots that mirror the keep/discard + running-best style
...
def _plot_comparison(original: pd.DataFrame, qiea: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title(
        "Benchmark Comparison (Running Best)\n"
        f"original_best={best_orig:.6f}, qiea_best={best_qiea:.6f}, "
        f"delta_best={delta:+.6f} (positive favors QIEA)",
        fontsize=13,
    )
    ax.set_xlabel("Run #", fontsize=12)
    ax.set_ylabel("Validation BPB (lower is better)", fontsize=12)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate progress-style benchmark plots from original and QIEA CSV results."
    )
    parser.add_argument(
        "--original-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to QIEA benchmark CSV",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
...
def main() -> None:
    ...
    qiea = _load_ok_results(args.qiea_csv.expanduser().resolve(), "QIEA")
    ...
    _plot_progress(qiea, "QIEA", "#27ae60", out_qiea)
